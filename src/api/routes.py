    ...
    if isinstance(content, str):
        ...
        for item in content:
            ...
            if item_type == "text":
                ...
                # extend://MEDIA_ID for video continuation
...
async def _append_openai_reference_images(
    ...
    debug_logger.log_info(f"[CONTEXT] Searching historical reference images, message count: {len(messages)}")
...
def _get_request_base_url(request: Request) -> Optional[str]:
    """Infer the externally accessible base URL from the actual request headers."""
