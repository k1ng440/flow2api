...
def _guess_client_hints_from_user_agent(user_agent: str) -> Dict[str, str]:
    """Infer common sec-ch-* headers from the User-Agent string."""
...
def _guess_impersonate_from_user_agent(user_agent: str) -> str:
    """Select the appropriate curl_cffi browser impersonation version based on the UA."""
    ua = (user_agent or "").strip()
    major_match = re.search(r"(?:Chrome|Chromium|Edg|EdgA|EdgiOS)/(\d+)", ua)
    if not major_match:
        return "chrome120"
...
def _normalize_http_base_url(base_url: str) -> str:
    ...
    if not normalized:
        raise RuntimeError("Remote captcha service URL not configured")

    parsed = urlparse(normalized)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise RuntimeError("Remote captcha service URL format invalid, must be http(s)://host[:port]")
...
async def _resolve_score_test_verify_proxy(
    ...
    """
    Select the verify-request proxy for score-test, preferring alignment with the browser captcha proxy.
...
async def _solve_recaptcha_with_api_service(
    ...
    """Obtain a reCAPTCHA token using the currently configured third-party captcha service."""
    ...
    raise RuntimeError(f"{method} token acquisition timed out")
...
async def _score_test_with_remote_browser_service(
    ...
    """Call the remote headed browser service to perform in-page captcha solving and score verification."""
    ...
    if not isinstance(response_payload, dict):
        raise RuntimeError("Remote captcha service response format invalid")
...
class AddTokenRequest(BaseModel):
    st: str
    project_id: Optional[str] = None  # Optional user-provided project_id
...
async def get_tokens(token: str = Depends(verify_admin_token)):
    ...
    return [{
        ...
        "token": row.get("at"),  # frontend compat: token.token access
        "email": row.get("email"),
        "name": row.get("name"),
...
async def add_token(
    ...
    try:
        ...
        # Hot-update concurrency limits without requiring service restart
...
async def update_token(
    ...
    """Update token - refreshes AT automatically from ST"""
    try:
...
async def refresh_at(
    ...
    """Manually refresh token AT (via ST conversion)"""
    ...
    try:
        ...
        if success:
            ...
            if config.captcha_method == "personal":
                message += " (ST auto-refresh supported)"
    ...
    for idx, item in enumerate(request.tokens):
        try:
            ...
            try:
                ...
                if not email:
                    errors.append(f"Item {idx+1}: could not retrieve email")
                    continue
...
async def test_proxy_connectivity(
    ...
    try:
        ...
        return {
            "success": ok,
            "message": "Proxy reachable" if ok else f"Proxy connectable but target returned status {status_code}",
            "test_url": test_url,
            "final_url": final_url,
...
async def update_call_logic_config(
    ...
    return {
        "success": True,
        "message": "Token call mode saved successfully",
        "config": {
            "call_mode": call_mode,
            "polling_mode_enabled": call_mode == "polling",
        }
...
async def update_token_refresh_enabled(
    ...
    """Update AT auto refresh enabled (always on in Flow2API, endpoint exists for frontend compatibility)"""
    return {
        "success": True,
        "message": "AT auto-refresh is always enabled in Flow2API and cannot be disabled"
    }
...
async def update_cache_config_full(
    ...
    if timeout is not None:
        ...
        if timeout < 0:
            raise HTTPException(status_code=400, detail="Cache timeout must not be negative")
...
async def update_captcha_config(
    ...
    try:
        ...
        return {"success": False, "message": "Reset solve count must be an integer; 0 to disable"}

    if captcha_method == "remote_browser":
        if not (remote_browser_base_url or "").strip():
            return {"success": False, "message": "remote_browser mode requires remote captcha service URL"}
...
async def test_captcha_score(
    ...
    """Obtain a token using the current captcha method and submit it to antcpt for score verification."""
        ...
        if captcha_method == "browser":
            ...
            if captcha_method == "capsolver" and "antcpt.com" in website_url:
                ...
                if token_value:
                    ...
                    return {
                        "success": True,
                        "message": "CapSolver does not support antcpt. Successfully tested connectivity via Google Labs.",
                        "captcha_method": captcha_method,
                        "website_url": "https://labs.google/",
                        "website_key": "6LdsFiUsAAAAAIjVDZcuLhaHiDn5nnHVXVRQGeMV",
                        "action": "IMAGE_GENERATION",
                        ...
                        "score": 0.9,
                        "verify_result": {"success": True, "message": "Score verification skipped"},
                        "verify_request_meta": {},
                        "browser_proxy_enabled": browser_proxy_enabled,
