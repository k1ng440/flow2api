"""
Local reCAPTCHA captcha service based on RT (standalone, no fake_useragent)
Supports: automatic Session Token refresh, external fingerprint rotation, persistent retry
"""
import os
import sys
import subprocess
import signal
# workaround for playwright asyncio compatibility on Windows
...
def _ensure_playwright_installed() -> bool:
    ...
    try:
        import playwright
        debug_logger.log_info("[BrowserCaptcha] playwright already installed")
        return True
    except ImportError:
        pass

    debug_logger.log_info("[BrowserCaptcha] playwright not installed, auto-installing...")
    print("[BrowserCaptcha] playwright not installed, auto-installing...")

    if _run_pip_install('playwright', use_mirror=False):
        return True

    debug_logger.log_info("[BrowserCaptcha] Official source failed, trying mirror...")
    print("[BrowserCaptcha] Official source failed, trying mirror...")
...
def _ensure_browser_installed() -> bool:
    ...
    try:
        ...
        if result.returncode == 0 and browser_path and os.path.exists(browser_path):
            debug_logger.log_info(f"[BrowserCaptcha] chromium browser already installed: {browser_path}")
            return True
    except Exception as e:
        debug_logger.log_info(f"[BrowserCaptcha] Error detecting browser: {e}")

    debug_logger.log_info("[BrowserCaptcha] chromium browser not installed, auto-installing...")
    print("[BrowserCaptcha] chromium browser not installed, auto-installing...")
    ...
    debug_logger.log_error("[BrowserCaptcha] ❌ chromium auto-install failed, please install manually: python -m playwright install chromium")
    print("[BrowserCaptcha] ❌ chromium auto-install failed, please install manually: python -m playwright install chromium")
    return False
...
if DOCKER_HEADED_BLOCKED:
    debug_logger.log_warning(
        "[BrowserCaptcha] Docker environment detected, headed browser captcha disabled by default. "
        "To enable, set ALLOW_DOCKER_HEADED_CAPTCHA=true and provide DISPLAY/Xvfb."
    )
    print("[BrowserCaptcha] ⚠️ Docker environment detected, headed browser captcha disabled by default")
    ...
    if IS_DOCKER and ALLOW_DOCKER_HEADED:
        debug_logger.log_warning(
            "[BrowserCaptcha] Docker headed browser captcha whitelist enabled, ensure DISPLAY/Xvfb is available"
        )
        print("[BrowserCaptcha] ✅ Docker headed browser captcha whitelist enabled")
...
def parse_proxy_url(proxy_url: str) -> Optional[Dict[str, str]]:
    """Parse proxy URL (socks5h:// is treated as socks5 in Playwright)."""
...
def normalize_browser_proxy_url(proxy_url: str) -> tuple[Optional[str], Optional[str]]:
    """Normalize the browser proxy URL to a format accepted by Playwright/Chromium.

    Chromium does not support socks5/socks5h proxy authentication with credentials.
    For `socks5(h)://user:pass@host:port`, automatically downgrades to `http://user:pass@host:port`
    for compatibility with providers that offer both HTTP and SOCKS5 endpoints.
    ...
    if protocol.startswith("socks5") and username and password:
        ...
        warning = (
            ...
            "Chromium does not support socks5 user:pass auth; "
            f"automatically using HTTP proxy: http://{host}:{port}"
        )
...
class TokenBrowser:
    """Simplified browser: launches a new browser for each token acquisition and closes it immediately.

    Uses a new random UA each time to avoid issues with long-running sessions.
    """
    ...
    async def _create_browser(self, token_proxy_url: Optional[str] = None, manage_slot_pid: bool = True) -> tuple:
        ...
        # Only record the proxy for now; let the browser expose its real UA/UA-CH to avoid version mismatch between user-agent and sec-ch-ua.
    ...
    async def _capture_page_fingerprint(self, page):
        """Extract UA and client-hint headers from the browser page to match the captcha browser."""
        try:
    ...
    async def _verify_score_in_page(self, page, token: str, verify_url: str) -> Dict[str, Any]:
        """Read the score directly from the test page DOM, avoiding inconsistency between verify.php and the page display."""
        _ = token
        _ = verify_url
    ...
    async def _wait_and_close_after_request(
        ...
        """Wait for the upstream request to finish before closing the browser (with timeout fallback)."""
        close_reason = "upstream request complete"
        try:
            await asyncio.wait_for(release_event.wait(), timeout=wait_timeout)
        except asyncio.TimeoutError:
            close_reason = f"timeout waiting for upstream request ({wait_timeout}s)"
            debug_logger.log_warning(
                f"[BrowserCaptcha] Token-{self.token_id} {close_reason}, performing fallback close"
            )
        except Exception as e:
            close_reason = f"exception waiting for upstream request: {type(e).__name__}"
            debug_logger.log_warning(
                f"[BrowserCaptcha] Token-{self.token_id} {close_reason}, performing fallback close"
            )
        finally:
    ...
    async def _defer_browser_close_until_request_done(
        ...
        """Delay closing the browser after a successful captcha solve, waiting for the Flow request to finish."""
        ...
        if action == "IMAGE_GENERATION":
            # Image pipeline may include upsample requests; wait limit must cover flow/upsample timeout
            base_timeout = max(flow_timeout, upsample_timeout)
            wait_timeout = max(base_timeout + 180, 900)
        else:
            # Video requests have longer timeouts; provide more buffer to avoid premature close
        ...
        debug_logger.log_info(
            f"[BrowserCaptcha] Token-{self.token_id} captcha succeeded, entering deferred close until upstream request finishes "
    ...
    async def notify_generation_request_finished(self, request_ref: Optional[str] = None):
        """Notify that the upstream image/video request for this token has finished."""
        async with self._pending_release_lock:
            release_event = None
            ...
            if matched_ref and matched_ref in self._pending_release_entries:
                ...
                # Backward-compatible: no request_ref provided; release the earliest pending entry only to avoid clearing all.
        ...
        if release_event and not release_event.is_set():
            release_event.set()
            debug_logger.log_info(
                f"[BrowserCaptcha] Token-{self.token_id} received upstream finish notification, closing browser "
    ...
    async def _execute_captcha(self, context, project_id: str, website_key: str, action: str) -> Optional[str]:
        """Execute the captcha solving logic in the given context."""
        page = None
        try:
            page = await context.new_page()
            await page.add_init_script("Object.defineProperty(navigator, 'webdriver', {get: () => undefined});")

            # Use a simpler API URL to avoid loading complex pages
            ...
            # Capture the actual UA/client-hints from this captcha page
            ...
            # Extra wait to ensure the enterprise request chain has fully settled
            post_wait_seconds = float(getattr(config, "browser_recaptcha_settle_seconds", 3) or 3)
            if post_wait_seconds > 0:
                debug_logger.log_info(
                    f"[BrowserCaptcha] Token-{self.token_id} token acquired, waiting {post_wait_seconds:.1f}s before returning"
                )
    ...
    async def _execute_custom_captcha(
        ...
        """Execute reCAPTCHA on any site, for score testing and non-Flow scenarios."""
        page = None
        try:
            page = await context.new_page()
            await page.add_init_script("Object.defineProperty(navigator, 'webdriver', {get: () => undefined});")
            ...
            for _ in range(20):
                ...
                await asyncio.sleep(0.5)
            if not page_loaded:
                debug_logger.log_warning(f"[BrowserCaptcha] Token-{self.token_id} custom page readyState did not reach complete, proceeding with warmup")

            # Simulate natural foreground interaction to avoid direct execute on a cold blank context.
            try:
            ...
            if warmup_seconds > 0:
                debug_logger.log_info(
                    f"[BrowserCaptcha] Token-{self.token_id} warming up real page for {warmup_seconds:.1f}s before custom captcha"
                )
            ...
            if post_wait_seconds > 0:
                debug_logger.log_info(
                    f"[BrowserCaptcha] Token-{self.token_id} custom captcha complete, waiting {post_wait_seconds:.1f}s before returning token"
                )
                await asyncio.sleep(post_wait_seconds)
    ...
    def get_last_fingerprint(self) -> Optional[Dict[str, Any]]:
        """Return a snapshot of the most recent captcha browser fingerprint."""
...
class BrowserCaptchaService:
    ...
    Supports configuring the number of browsers; each browser opens one tab; requests are round-robin distributed.
    """
    ...
    def _check_available(self):
        ...
        if DOCKER_HEADED_BLOCKED:
            raise RuntimeError(
                "Docker environment detected, headed browser captcha disabled by default. "
                "Set ALLOW_DOCKER_HEADED_CAPTCHA=true and provide DISPLAY/Xvfb to enable."
            )
        if IS_DOCKER and not os.environ.get("DISPLAY"):
            raise RuntimeError(
                "Docker headed browser captcha is enabled but DISPLAY is not set. "
    ...
    async def _load_browser_count(self):
        """Load browser count configuration from the database."""
        if self.db:
            try:
                captcha_config = await self.db.get_captcha_config()
                ...
                self._browser_count = 1
        # Concurrency limit = browser count, no longer hardcoded
    ...
    async def reload_browser_count(self):
        """Reload browser count from config and reconcile running slots."""
            ...
            async with self._browsers_lock:
                for browser_id in list(self._browsers.keys()):
                    if browser_id >= self._browser_count:
                        ...
                        debug_logger.log_info(f"[BrowserCaptcha] removed browser slot {browser_id}")
        ...
        for browser in browsers_to_close:
            try:
                ...
                debug_logger.log_warning(f"[BrowserCaptcha] failed to close removed browser slot: {e}")
    ...
    async def _select_browser_id(self, project_id: Optional[str]) -> int:
        # browser mode no longer pins a slot per project_id.
        # Prefer idle warmed slots, then idle cold slots; fall back to round-robin when all busy.
        async with self._slot_allocation_lock:
            async with self._browsers_lock:
    ...
    def _get_next_browser_id(self) -> int:
        """Round-robin: get next browser ID."""
    ...
    def _parse_browser_ref(browser_ref: Optional[Union[int, str]]) -> tuple[Optional[int], Optional[str]]:
        """Parse a request handle, backward-compatible with plain int browser_id."""
    ...
    async def _resolve_token_proxy_url(self, token_id: Optional[int]) -> Optional[str]:
        """Read the token-level captcha proxy; fall back to global config if absent."""
        ...
        try:
            ...
            debug_logger.log_warning(f"[BrowserCaptcha] Failed to read captcha proxy for token({token_id}): {e}")
        return None
    
    async def get_token(self, project_id: str, action: str = "IMAGE_GENERATION", token_id: int = None) -> tuple[Optional[str], Union[int, str]]:
        """Get a reCAPTCHA token (selects a slot from the shared browser pool).

        Args:
            project_id: Project ID
            action: reCAPTCHA action
            token_id: Business token ID (used only to read the token-level captcha proxy)
    ...
    async def get_custom_token(
        ...
        """Get a reCAPTCHA token for any site, for score testing."""
        self._check_available()
    ...
    async def get_fingerprint(self, browser_ref: Optional[Union[int, str]]) -> Optional[Dict[str, Any]]:
        """Get the fingerprint snapshot from the most recent captcha solve for the given browser."""
    ...
    async def report_request_finished(self, browser_ref: Optional[Union[int, str]] = None):
        """Called by the caller to signal the request is done; browser mode keeps the shared browser alive and does not close it after success."""
        browser_id, _ = self._parse_browser_ref(browser_ref)
        if browser_id is None:
            return
