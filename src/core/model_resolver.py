...
# If requested ratio is not in supported list, downgrade to default
...
# - imageSize here refers to flow2api's upscale tier, not an OpenAI pixel dimension;
#   used as an approximate mapping for quality/clarity.
OPENAI_QUALITY_MAP = {
    "low": None,
    "standard": None,
    "medium": "2k",
...
# Maps OpenAI size (e.g. 1024x1792) to the closest flow2api aspect option
...
def _extract_generation_params(request) -> Tuple[Optional[str], Optional[str]]:
    ...
    if gen_config is not None:
        ...
        # some upstreams place fields at the generationConfig top level
        if not aspect_ratio:
            aspect_ratio = _normalize_aspect_ratio(
    ...
    # 2) if not found at top level, try generationConfig passed through extra fields (Pydantic extra="allow")
    ...
    if (aspect_ratio is None or image_size is None) and hasattr(request, "__pydantic_extra__"):
        ...
        # some upstreams may pass aspect_ratio/image_size directly
        if aspect_ratio is None:
...
def resolve_model_name(
    ...
    """Resolve a simplified model name + generationConfig params to an internal MODEL_CONFIG key.

    If model is already a valid MODEL_CONFIG key, return it directly.
    If model is a simplified name (base model name), construct the full internal model name
    from the aspectRatio / imageSize in generationConfig.

    Args:
        model: model name from the request
        request: ChatCompletionRequest instance (used to extract generationConfig)
        model_config: MODEL_CONFIG dict (used to validate the resolved model name)
    ...
    if model in IMAGE_BASE_MODELS:
        ...
        debug_logger.log_info(
            f"[MODEL_RESOLVER] model name converted: {model} → {resolved} "
    ...
    if model in VIDEO_BASE_MODELS:
        ...
        if resolved and model_config and resolved in model_config:
            debug_logger.log_info(
                f"[MODEL_RESOLVER] video model name converted: {model} → {resolved} "
    ...
    # already a valid MODEL_CONFIG key, return as-is
    if model_config and model in model_config:
        return model

    # unknown model name, return as-is (downstream MODEL_CONFIG lookup will error)
    return model


def get_base_model_aliases() -> Dict[str, str]:
    """Return all simplified model names (aliases) and descriptions, for display in the /v1/models endpoint."""
    aliases = {}
